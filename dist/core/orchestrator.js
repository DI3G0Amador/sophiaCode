 ...
 */
export async function runInitFlow(basePath) {
    ...
    try {
        ...
        p.log.error(`Não foi possível salvar o config.json temporário: ${error.message}`);
    }
    ...
    try {
        ...
        try {
            ...
            const documentation = await aiService.generateStructured(SYSTEM_PROMPT, userPrompt, responseSchema);
            ...
            await saveRootBridgedFiles(basePath, documentation.claudeContent, documentation.rootAgentsContent, setupConfig.integrations);
            ...
            p.log.error(`Erro: ${error.message}`);
            p.outro('A configuração falhou.');
        }
    }
    catch (error) {
        scanSpinner.stop('A análise local falhou!');
        p.log.error(`Erro na análise estática: ${error.message}`);
        p.outro('A configuração falhou.');
    }
}
