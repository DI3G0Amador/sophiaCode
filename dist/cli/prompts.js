 ...
 */
export async function askSetupConfig(defaultValues) {
    ...
    const rules = await p.group({
        ...
        outOfScope: () => p.text({
            ...
            placeholder: 'Ex: Não enviar mensagens de conversas comuns, não coletar respostas de usuários',
            initialValue: defaultValues?.outOfScope,
        }),
        acceptableErrors: () => p.text({
        ...
        provider: () => p.select({
            ...
            options: [
                { value: 'gemini', label: 'Google Gemini (Nuvem - recomendado)', hint: 'Requer GEMINI_API_KEY' },
                { value: 'openai', label: 'OpenAI (Nuvem)', hint: 'Requer OPENAI_API_KEY' },
                { value: 'ollama', label: 'Ollama (Local / Offline)', hint: 'Requer o Ollama rodando localmente' }
            ]
        }),
        modelName: ({ results }) => {
            ...
            const initialModel = defaultValues?.provider === results.provider
        ...
        integrations: () => p.multiselect({
            message: 'Deseja criar pontes de integração na raiz do projeto para outros agentes?',
            initialValues: defaultValues?.integrations || [],
            options: [
                { value: 'claude', label: 'Claude Code (Cria/Edita CLAUDE.md na raiz)', hint: 'Redireciona o Claude Code para o sophiAgents' },
                { value: 'opencode', label: 'OpenCode / Outros (Cria/Edita AGENTS.md na raiz)', hint: 'Redireciona o OpenCode para o sophiAgents' }
            ],
