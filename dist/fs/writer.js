 ...
 */
function getConfigDirectory(basePath) {
 ...
 */
export async function checkConfigExist(basePath) {
 ...
 */
export async function saveDocumentation(basePath, mapContent, agentsContent) {
    const configDir = getConfigDirectory(basePath);
    const mapPath = path.join(configDir, 'MAP.md');
    const agentsPath = path.join(configDir, 'Agents.md');
 ...
 */
async function writeOrRedirectFile(filePath, fileName, newContent) {
    try {
        ...
        const redirectedContent = `# SophiaCode Redirection\n\n` +
            `> [!IMPORTANT]\n` +
 ...
 */
export async function saveRootBridgedFiles(basePath, claudeContent, rootAgentsContent, integrations = []) {
 ...
 */
export async function readMapFile(basePath) {
 ...
 */
export async function writeMapFile(basePath, content) {
    const mapPath = path.join(getConfigDirectory(basePath), 'MAP.md');
 ...
 */
export async function saveProjectConfig(basePath, config) {
    const configDir = getConfigDirectory(basePath);
 ...
 */
export async function readProjectConfig(basePath) {
